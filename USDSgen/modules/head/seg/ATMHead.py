...
class ATMHead(BaseDecodeHead):
    ...
    def forward(self, inputs):
        ...
        for idx, (x_, proj_, norm_, decoder_) in enumerate(
            zip(x, self.input_proj, self.proj_norm, self.decoder)
        ):
            ...
            if self.crop_train and self.training:
                ...
                blank_attn = (
                    blank_attn[:, 0]
                    .unsqueeze(1)
                    .repeat(1, (self.image_size // 16) ** 2, 1)
                )
    ...
    def forward_with_loss(self, inputs, labels):
        """
        Forward function with loss computation.
        Inputs: batch (dict) with keys: image, mask
        Outputs: loss, outputs,
        """
        outputs = self.forward(inputs)
        loss = self.losses(outputs, labels)
        reduced_loss = 0
        for loss_name in loss.keys():
            reduced_loss += loss[loss_name]
        return reduced_loss, outputs["pred"], labels
